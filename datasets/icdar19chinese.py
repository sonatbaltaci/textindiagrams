...
def read_coordinates_from_xtml(path_label):
    ...
    NS = {'page': 'http://schema.primaresearch.org/PAGE/gts/pagecontent/2017-07-15'}


    # List to hold all extracted coordinates from every <Coords> element
    text_line_coordinates = []

    # Find all TextLine elements
    text_lines = root.findall('.//page:TextLine', NS)
    for text_line in text_lines:
        # Find the Coords child of each TextLine
        coords_el = text_line.find('page:Coords', NS)
        if coords_el is not None:
            points_str = coords_el.get('points')
            point_pairs = points_str.split()
            
            # Convert each "x,y" into (x, y) integer tuples
            coords_tuples = [tuple(map(int, pair.split(','))) for pair in point_pairs]
            coords_lists = [list(t) for t in coords_tuples]
            coords_list = np.array(coords_lists).reshape(-1)

            text_line_coordinates.append(coords_list)
    text_line_coordinates = np.array(text_line_coordinates)
    return text_line_coordinates
...
class ICDAR2019CHINESE(Dataset):
    def __init__(self, mode, transform=transforms.ToTensor(), target_transform=None, path_to_data=None,fold = 1):
       # folder 
        ...
        # self.list_files = os.listdir( os.path.join(self.path_to_data, 'images'))

        # self.list_files = [f for f in self.list_files if f.endswith('.png') or f.endswith('.jpg')]
        self.path_to_data = path_to_data
        train_combination = np.loadtxt(os.path.join(self.path_to_data, 'kfold', 'train_combination.txt'.format(fold)), dtype=float)
        train_combination = train_combination.astype(int)
        train_combination = train_combination[fold]
        all_folds = [0, 1, 2, 3, 4]
        train_combo_int = [int(x) for x in train_combination]
        remaining_folds = [f for f in all_folds if f not in train_combo_int]
        test_fold = max(remaining_folds) 
        val_fold = remaining_folds[0]
        if mode == 'train':
            train_split = []
            for i in range(len(train_combination)):
 
                path_fold = os.path.join(self.path_to_data, 'kfold', 'fold_{}'.format(train_combination[i]) + '.txt')
                with open(path_fold) as f:
                    list_files = f.readlines()
                train_split.extend([x.strip() for x in list_files])
            self.list_files = train_split
        elif mode == 'test':

            path_fold = os.path.join(
                self.path_to_data, 'kfold', f'fold_{test_fold}.txt')
            
            with open(path_fold, 'r') as f:
                test_split = [line.strip() for line in f.readlines()]
            self.list_files = test_split

            # take the greater fold that is not in the train_combination
        elif mode == 'val':
            #take the fold that is not in the train_combination and not the greater fold reamaning
            
            path_fold = os.path.join(
                self.path_to_data, 'kfold', f'fold_{val_fold}.txt')
            with open(path_fold, 'r') as f:
                val_split = [line.strip() for line in f.readlines()]
            self.list_files = val_split
        self.path_to_data =  os.path.join(path_to_data,'train')
    ...
    def __getitem__(self, idx):
        ...
        path_img = os.path.join(self.path_to_data, 'images', path_img)
        image = Image.open(path_img).convert("RGB")
        xml_file =  os.path.join(self.path_to_data,'xtml',path_img.split('/')[-1].replace("png","xml").replace("jpg","xml"))
        coordinates = read_coordinates_from_xtml(xml_file)
...
def build_ICDAR2019CHINESE(image_set, args,path_to_data):
    transforms = make_coco_transforms(image_set, args=args)
    return ICDAR2019CHINESE(image_set, transforms,path_to_data = path_to_data, fold = args.fold)
