...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
                    ...
                    logger=None, ema_m=None, run=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header, logger=logger):
        ...
        if args.wandb:
            ...
            for ll in loss_dict:
                run.add_scalar(ll, loss_dict[ll], iterations+len(data_loader) * epoch)
...
@torch.no_grad()
def evaluate(model, criterion, postprocessors, data_loader, base_ds, device, wo_class_error=False, args=None, logger=None, run=None):
    need_tgt_for_training = args.use_dn
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header, logger=logger):
        ...
        with torch.cuda.amp.autocast(enabled=args.amp):
            ...
            loss_dict = criterion(outputs, targets)
        # add val prefix to loss keys
        loss_wandb_dict = {'val_'+k: v for k, v in loss_dict.items()}
        if args.wandb:
            run.log(loss_wandb_dict)
        weight_dict = criterion.weight_dict
...
@torch.no_grad()
def test(model, criterion, postprocessors, data_loader, device, args=None, logger=None):
    model.eval()
    criterion.eval()
