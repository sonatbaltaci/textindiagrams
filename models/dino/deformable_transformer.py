    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        if self.two_stage_type =='standard':
            if self.two_stage_learn_wh:
                ...
                input_hw = None
            output_memory, output_proposals = gen_encoder_output_proposals(memory, mask_flatten, spatial_shapes, input_hw, query_dim=self.query_dim)
            ...
            refpoint_embed_undetach = torch.gather(enc_outputs_coord_unselected, 1, topk_proposals.unsqueeze(-1).repeat(1, 1, self.query_dim)) # unsigmoid
    ...
    def forward(self, tgt, memory,
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.deformable_decoder:
                if reference_points.shape[-1] == self.query_dim:
                    reference_points_input = reference_points[:, :, None] \
                                            * torch.cat([valid_ratios]*(self.query_dim//2), -1)[None, :] # nq, bs, nlevel, 4
                else:
                    assert reference_points.shape[-1] == 2
                    reference_points_input = reference_points[:, :, None] * valid_ratios[None, :]
                query_sine_embed = gen_sineembed_for_position(reference_points_input[:, :, 0, :], query_dim=self.query_dim) # nq, bs, 256*2 
            else:
