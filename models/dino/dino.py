...
class DINO(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        _bbox_embed = MLP(hidden_dim, hidden_dim, self.query_dim, 3) ## 4->8 ->12
        ...
        if self.dn_number > 0 or targets is not None:
            input_query_label, input_query_bbox, attn_mask, dn_meta =\
                prepare_for_cdn(dn_args=(targets, self.dn_number, self.dn_label_noise_ratio, self.dn_box_noise_scale),
                                ...
                                hidden_dim=self.hidden_dim,label_enc=self.label_enc, query_dim=self.query_dim)
        else:
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        # mask
        have_mask = any("mask_ce" in t for t in targets)
        if have_mask:
            target_mask_o = torch.cat([t["mask_ce"][J] for t, (_, J) in zip(targets, indices)]).to(torch.int64)
            mask_ce = torch.ones(src_logits.shape[:2], dtype=torch.int64, device=src_logits.device)
            mask_ce[idx] = target_mask_o
            mask_ce = mask_ce.unsqueeze(-1) 
            loss_ce = sigmoid_focal_loss(src_logits, target_classes_onehot, num_boxes, alpha=self.focal_alpha, gamma=2, mask_ce=mask_ce) * src_logits.shape[1]
        else:
